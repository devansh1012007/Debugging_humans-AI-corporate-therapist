...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        l_ceo, _ = StructureLevel.objects.get_or_create(company=comp, name="CEO", level_rank=1)
        l_head, _ = StructureLevel.objects.get_or_create(company=comp, name="Department Head", level_rank=2)
        l_lead, _ = StructureLevel.objects.get_or_create(company=comp, name="Team Lead", level_rank=3)
        l_assoc, _ = StructureLevel.objects.get_or_create(company=comp, name="Associate", level_rank=4)

        # --- 2. Data Definitions (Indian Context) ---
        
        individual_positives = [
            "Demonstrates high resilience under pressure.",
            "Maintains a healthy work-life boundary.",
            "Excellent peer support and empathy.",
            "Consistently utilizes wellness breaks.",
            "High psychological safety scores.",
            "Active participant in team bonding."
        ]
        
        individual_problems = [
            {"problem": "Imposter Syndrome", "description": "User feels undervalued despite high performance metrics."},
            {"problem": "Commute Fatigue", "description": "Long travel times in Bangalore traffic affecting energy levels."},
            {"problem": "Digital Overload", "description": "High screen time average (>10 hours) leading to eye strain."},
            {"problem": "Financial Anxiety", "description": "Stress markers related to appraisal season uncertainty."},
            {"problem": "Isolation", "description": "Feeling disconnected due to 100% remote working setup."},
            {"problem": "Sleep Deprivation", "description": "Logins detected late at night consistently."}
        ]

        individual_recs = [
            {"recommendation": "Encourage taking a 'Mental Health Day'."},
            {"recommendation": "Suggest the 20-20-20 rule for eye strain."},
            {"recommendation": "Schedule a 1:1 mentorship session."},
            {"recommendation": "Recommend the internal 'Mindful Mornings' workshop."},
            {"recommendation": "Disconnect from Slack after 7 PM."}
        ]

        team_problems = [
            {"problem": "Meeting Fatigue", "description": "Team spends >60% of time on calls, leaving no focus time."},
            {"problem": "Siloed Knowledge", "description": "Junior members feel hesitant to ask questions in public channels."},
            {"problem": "Crunch Time Burnout", "description": "Elevated stress levels detected following the Q3 product launch."},
            {"problem": "Timezone Dissonance", "description": "Late night calls with US clients causing sleep disruption."},
            {"problem": "Lack of Recognition", "description": "Team morale dip due to lack of visible appreciation."}
        ]

        team_recs = [
            {"recommendation": "Implement 'No-Meeting Wednesdays'."},
            {"recommendation": "Organize a casual chai-break virtual hangout."},
            {"recommendation": "Review resource allocation for the upcoming sprint."},
            {"recommendation": "Mandatory post-launch recovery time off."},
            {"recommendation": "Start a 'Kudos' channel for peer recognition."}
        ]

        team_policies = [
            {"title": "Right to Disconnect", "description": "No expectation to reply to emails after 8 PM IST."},
            {"title": "Wellness Allowance", "description": "Budget allocated for gym memberships or therapy apps."},
            {"title": "Hybrid Roster", "description": "Flexible 2-day office policy to reduce commute stress."},
            {"title": "Focus Hours", "description": "1 PM to 4 PM is blocked for deep work; no internal calls."}
        ]

        # --- 3. Employee Roster ---
        employees = [
            ("Aarav Patel", "aarav", l_ceo, None),
            ("Diya Sharma", "diya", l_head, "aarav"),   
            ("Vihaan Gupta", "vihaan", l_head, "aarav"), 
            ("Aditi Rao", "aditi", l_head, "aarav"),     
            ("Ishaan Verma", "ishaan", l_lead, "diya"), 
            ("Rohan Mehta", "rohan", l_lead, "diya"),   
            ("Sanya Malhotra", "sanya", l_lead, "vihaan"), 
            ("Kabir Singh", "kabir", l_lead, "aditi"), 
            ("Arjun Nair", "arjun", l_assoc, "ishaan"),
            ("Ananya Iyer", "ananya", l_assoc, "ishaan"),
            ("Dev Patel", "dev", l_assoc, "ishaan"),
            ("Meera Reddy", "meera", l_assoc, "rohan"),
            ("Pranav Joshi", "pranav", l_assoc, "rohan"),
            ("Riya Kapoor", "riya", l_assoc, "sanya"),
            ("Vivaan Kumar", "vivaan_k", l_assoc, "sanya"),
            ("Kavya Jain", "kavya", l_assoc, "sanya"),
            ("Niharika Das", "niharika", l_assoc, "kabir"),
            ("Sai Krishna", "sai", l_assoc, "kabir"),
        ]

        node_map = {}
        self.stdout.write("Creating Users and Nodes...")

        for full_name, username, level_obj, parent_username in employees:
            ...
            parent_node = node_map.get(parent_username) if parent_username else None

            # --- THE FIX IS HERE: 'structure_level': level_obj ---
            node, _ = OrgNode.objects.get_or_create(
                user=user,
                defaults={
                    'name': full_name,
                    'company': comp,
                    'structure_level': level_obj, 
                    'parent': parent_node
                }
            )
            
            node_map[username] = node

            my_positives = random.sample(individual_positives, 3)
            my_problems = random.sample(individual_problems, 2)
            my_recs = random.sample(individual_recs, 3)

            UserDashboard.objects.update_or_create(
                owner=user,
                defaults={'content': {
                    "_personal_dashboard_data": {
                        "positive": [{"positive": p} for p in my_positives],
                        "common_problems": my_problems,
                        "recommendation": my_recs
                    }
                }}
            )

            if level_obj.level_rank < 4:
                t_probs = random.sample(team_problems, 3)
                t_recs = random.sample(team_recs, 3)
                t_pols = random.sample(team_policies, 2)

                TeamData.objects.update_or_create(
                    node=node,
                    defaults={
                        'content': {
                            "common_problems": t_probs,
                            "recommendations": t_recs,
                            "policy_changes": t_pols
                        }
                    }
                )

                self.stdout.write(f"Generating drill down for {full_name}...")
                generate_drill_down_lists(node)

        self.stdout.write(self.style.SUCCESS(f'Successfully created {len(employees)} employees with realistic data!'))
    ...
    def handle(self, *args, **kwargs):
        ...
        self.create_user_dashboard(
            ...
            problems=[
                ...
                {"problem": "Rejection Accumulation", "description": "High volume of 'no' responses taking emotional toll despite individual resilience"},
        ...
        # LEVEL 5: EMPLOYEES
        # Create employees under each team lead
